...
def save_model_figure_df_train_test(df, df_filtered, out_path, target_column_categorized, drop_col_class, positive_class='active'):
    ...
    Adaptado para DecisionTreeClassifier
    ...
    for _, row in df_filtered.iterrows():
        ...
        params_str = f"{name}_criterion-{criterion}_min_split-{min_samples_split}_min_leaf-{min_samples_leaf}_kf-{n_splits}_rs-{random_state_kf}_test-{test_size}_split-{random_state_split}"
...
out_path = f"{pathway_dir}/3-machine_learning_models/supplementary_results/classification/models_analysis_decision_tree/"
...
top_models = results_df[results_df.name.str.contains("FRET")].query('train_acc_no_cv > 0.6 and test_acc > 0.6').sort_values(by=["test_roc_auc"], ascending=False).drop_duplicates(subset=['name', 'criterion', 'kf','test_size_split', 'random_state_split'], keep="first").head(30)
