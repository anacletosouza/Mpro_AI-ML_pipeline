...
output_dir = f'{pathway_dir}/2-computation_molecular_descriptors/'
datasets = f'{pathway_dir}/1-preprocessing/'
path_datasets = f'{pathway_dir}/1-preprocessing/1-datasets/'
path_class = f"{pathway_dir}/2-computation_molecular_descriptors/dataset_non-correlation_desc_reduced_dim/classification"
...
def calculate_descriptors(df, name):
    output_file = f"{pathway_dir}/2-computation_molecular_descriptors/dataset_non-correlation_desc_reduced_dim/original_data/"
...
def rm_bad_col(df, drop_col, target_col="pIC50_class"):
...
def filter_col_correl(df_r, threshold=0.4, target_col="pIC50_class"):
...
for category, data in processed_descriptors_class.items():
    for dim, df in data.items():
        for threshold in thresholds:
            ...
            output_file = f"{pathway_dir}/2-computation_molecular_descriptors/dataset_non-correlation_desc_reduced_dim/classification/df_{category}_{dim}_threshold_{threshold}_class.tsv"
