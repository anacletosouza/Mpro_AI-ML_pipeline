                ...
                for k in random_state_list:
                    for m in test_size_split_list:
                        for solver in solver_list:
                            tasks.append((name, df, i, j, m, k, solver))
    return tasks

def train_model(args):
    name, df, n_splits, random_state_kf, test_size_split, random_state_split, solver = args
    ...
    model_no_cv = LogisticRegression(solver=solver, random_state=random_state_split, max_iter=1000)
    ...
    for train_index, val_index in kf.split(X_train):
        ...
        model = LogisticRegression(solver=solver, random_state=random_state_kf, max_iter=1000)
    ...
    model_final = LogisticRegression(solver=solver, random_state=random_state_split, max_iter=1000)
...
while datasets:
    ...
    if count > 2:
        ...
        results_df.to_csv(f"{pathway_dir}/3-machine_learning_models/supplementary_results/classification/hyperparameters_models_performance/results_conditions_models_LogisticRegression.tsv", sep="\t", index=False)
        print(f"Count: {count}")
        break
